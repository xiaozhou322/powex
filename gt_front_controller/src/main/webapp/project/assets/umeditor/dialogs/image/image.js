...
(function() {
  ...
  var Upload = {
    ...
    submit: function(callback) {
      var me = this,
        input = $('<input style="filter: alpha(opacity=0);" class="edui-image-file" id="eduifile" type="file" hidefocus="" name="file" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'),
        input = input[0];
             ...
             ifstate = 0;
               $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function() {
                 if(ifstate === 1) {
                   ...
                   ifstate = 1;
                   this.contentWindow.location = "/assets/umeditor/dialogs/image/temp.html";
                 }
               });*/
    ...
    /*    $(this).parent()[0].submit();
        ...
        // var formData = new FormData($(".edui-image-file").parent()[0]);
        var formData = new FormData();
        formData.append('file', filqe);
        $.ajax({
          // url : "http://192.168.2.16:8188/message/upload",
        //   url : "http://10.3.110.173:8085/operation/message/upload",
           url : "/operation/message/upload",
        //   url : "http://192.168.2.175:8085/operation/message/upload",
        //    url : "http://operation.jsyinhao.com/operation/message/upload",
        //    url : "http://47.106.99.88:86",
          type : "POST",
          data : formData,
          cache: false,
          processData: false,
          ...
          success : function(data) {
           // var  data = 'http://192.168.2.170'+data["data"];
            var returnJson =  eval(data);
            if (!/\.(gif|jpg|jpeg|png|GIF|JPG|PNG)$/.test(returnJson["data"])) {
              /// nui.alert("图片类型必须是.gif,jpeg,jpg,png中的一种","系统提示");
              alert("图片类型必须是.gif,jpeg,jpg,png中的一种")
            } else {
              newUrl = returnJson["data"];
              // newUrl = returnJson[0].fileURL;
              // newFileName = returnJson[0].name;
              //显示图片计数+1
              Upload.showCount++;
             //  var $img = $("<img src='" +'http://jdimages.jsyinhao.com/'+ newUrl + "' class='edui-image-pic' />"),
             //  var $img = $("<img src='" +'http://47.106.99.88:86'+ newUrl + "' class='edui-image-pic' />"),
             //   var $img = $("<img src='" +'http://10.3.110.170/'+ newUrl + "' class='edui-image-pic' />"),
               var $img = $("<img src='" +'fdfs'+ newUrl + "' class='edui-image-pic' />"),
                $item = $("<div class='edui-image-item edui-image-upload-item'><div class='edui-image-close'></div></div>").append($img);

              if ($(".edui-image-upload2", $('#edui-dialog-image')).length < 1) {
                $(".edui-image-content", $('#edui-dialog-image')).append($item);

                Upload.render(".edui-image-content", 2)
                  .config(".edui-image-upload2");
              } else {
                $(".edui-image-upload2", $('#edui-dialog-image')).before($item).show();
              }

              $img.on("load", function () {
                Base.scale(this, 120);
                Base.close($(this));
                $(".edui-image-content", $('#edui-dialog-image')).focus();
              });
              Upload.toggleMask();
            }
