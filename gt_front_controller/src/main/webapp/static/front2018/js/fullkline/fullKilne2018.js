...
function(t) {
	...
	M.prototype.__construct = function() {
		...
		for (var r = t.toFixed(10), a = r.length - 1; a >= 0; a--) {
	...
	E.prototype.__construct = function(t, e) {
		...
		return 0 == e ? e : 0 == r ? e > 0 ? 1e10 : -1e10 : e / r
	};
	var W = r(F),
		G = r(F),
		z = r(F),
	...
	Jt.prototype.__construct = function(t) {
		...
		if (!(this._areas.length < 1)) {
			...
			if (!e.getCaptureMouseWheelDirectly()) for (a = 0, i += 2; a < i; a += 2) if (this._areas[a].isSelected()) {
				t.strokeStyle = r.getColor(de.Color.Background), t.strokeRect(this.getLeft() + .5, this.getTop() + .5, this.getWidth() - 1, this.getHeight() - 1);
				break
			}
		}
	...
	De._dsAliasToString = {
		...
		if (!(h < 0)) {
			...
			if ("zh-cn" == m) {
				...
				if (!we.drawString(t, "  开: " + c.open.toFixed(10), n)) return;
				if (!we.drawString(t, "  高: " + c.high.toFixed(10), n)) return;
				if (!we.drawString(t, "  低: " + c.low.toFixed(10), n)) return;
				if (!we.drawString(t, "  收: " + c.close.toFixed(10), n)) return
			} else if ("en-us" == m) {
				if (!we.drawString(t, "DATE: " + _ + "-" + p + "-" + d + "  " + g + ":" + f, n)) return;
				if (!we.drawString(t, "  O: " + c.open.toFixed(10), n)) return;
				if (!we.drawString(t, "  H: " + c.high.toFixed(10), n)) return;
				if (!we.drawString(t, "  L: " + c.low.toFixed(10), n)) return;
				if (!we.drawString(t, "  C: " + c.close.toFixed(10), n)) return
			} else if ("zh-tw" == m) {
				if (!we.drawString(t, "時間: " + _ + "-" + p + "-" + d + "  " + g + ":" + f, n)) return;
				if (!we.drawString(t, "  開: " + c.open.toFixed(10), n)) return;
				if (!we.drawString(t, "  高: " + c.high.toFixed(10), n)) return;
				if (!we.drawString(t, "  低: " + c.low.toFixed(10), n)) return;
				if (!we.drawString(t, "  收: " + c.close.toFixed(10), n)) return
			}
	...
	Le.prototype.__construct = function(t) {
		...
		if (0 == this.m_mode || 2 == this.m_mode) {
			t.strokeStyle = this.m_pTheme.getColor(de.Color.Background), t.beginPath(), t.moveTo(this.m_bid_points[0].x + .5, this.m_bid_points[0].y + .5);
	...
	Ee.state = {
		...
		var d = Math.abs(l - s),
			...
			f = Math.abs(_ - s),
			m = Math.abs(p - s);
		return d <= g && d <= f && d <= m && d < 10 && (a = n.open), g <= d && g <= f && g <= m && g < 10 && (a = n.high), f <= d && f <= g && f <= m && f < 10 && (a = n.low), m <= d && m <= g && m <= f && m < 10 && (a = n.close), a
	};
	var Xe = r(Xt);
	Xe.state = {
		BeforeDraw: 0,
