var security = {
	loadgoogleAuth : function() {
		...
		$.post(url, param, function(data) {
			if (data.code == 0) {
				...
				document.getElementById("bindgoogle-key1").innerHTML = data.totpKey;
	...
	submitValidatePhoneForm : function(isUpdate, nvcData, callbackName) {
	...
	submitBindPhone : function(isUpdate, nvcData, callbackName) {
	...
	submitbindGoogleForm : function(nvcData, callbackName) {
		...
		jQuery.post(url, param, function(data) {
			...
			/*	util.showerrortips('bindgoogle-errortips', data.msg);
				...
				$(".googleBtn").click(function() {
					util.showerrortips('bindgoogle-errortips', data.msg);
					$(".ngrealModule").hide();
					window.location.href = '/introl/mydivide.html';
				})
	...
	submitPwdForm : function(pwdType, istrade, nvcData, callbackName) {
		var originPwdEle = "";
		var newPwdEle = "";
		var reNewPwdEle = "";
		var phoneCodeEle = "";
...
$(function() {
	...
	$("#unbindemail-Btn").on("click", function() {
		var email = $("#bindemail-email").val();
		security.submitValidateEmailForm(email, false);
	});
	...
	$("#unbindphone-Btn").on("click", function() {
		...
		security.submitValidatePhoneForm(1, nvcData, callbackName);
	});
	...
	$("#bindphone-Btn").on("click", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitBindPhone(0, nvcData, callbackName);
	});
	$("#bindgoogle-Btn").on("click", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitbindGoogleForm(nvcData, callbackName);
	});
	...
	$("#unbindloginpass-Btn").on("click", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitPwdForm(0, true, nvcData, callbackName);
	});
	$("#unbindtradepass-Btn").on("click", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitPwdForm(1, true, nvcData, callbackName);
	});
	$("#bindtradepass-Btn").on("click", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitPwdForm(1, false, nvcData, callbackName);
	});
	$("#bindtradepass-newpass").on("change", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitPwdForm(1, false, nvcData, callbackName);
	});
	$("#bindtradepass-confirmpass").on("change", function() {
		var nvcData = getNVCVal();
		var callbackName = ('jsonp_' + Math.random()).replace('.', '');
		security.submitPwdForm(1, false, nvcData, callbackName);
	});
