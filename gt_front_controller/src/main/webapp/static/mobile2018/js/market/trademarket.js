	...
	autoRefresh : function() {
		...
		var tradesCallback = function(result) {
			...
			$.each(data.trades, function(key, value) {
				...
				loghtml += '<li class="clear" style="text-align:center;">';
				loghtml += '<em>' + value.time + '</em>';
				loghtml += '<em class="s_n '+ (value.en_type == 'ask' ? "cgreen2" : "cred") +'">' + (value.en_type == 'ask' ? util.getLan("mark.sell") : util.getLan("mark.buy")) + '</em>';
				loghtml += '<em>' + util.numFormat(Number(value.price), market.cnyDigit) + '</em>';
				loghtml += '<em class="s_n' + (value.en_type == 'ask' ? "sellColor" : "buyColor") + '">' + util.numFormat(Number(value.amount), market.coinDigit) + '</em>';
				loghtml += '</li>';
			});
			if(loghtml == ''){
				var loghtm = "<div class='textCenter mtop'>"+  util.getLan("tradeMarket.tips.17") + "</p></div>";
			...
			$.each(data.buys, function(key, value) {
				...
				if (buyele.length == 0) {
					...
					html += '<div class="buyTit  depth-des market-font-buy list fl textL li_num"><span>' + parseInt(key + 1) + '</span></div>';
					html += '<div class="t_con"><span class="depth-price s_01 coin_price">' + util.numFormat(priceOffset(Number(value.price), 1), market.cnyDigit) + '</span></div>';
					html += '<div class="t_con"><span class="depth-amount s_01">' + util.numFormat(amountOffset(Number(value.amount), 1), market.coinDigit) + '</span></div>';
			...
			$.each(data.sells, function(key, value) {
				...
				if (sellele.length == 0) {
					...
					html += '<div class="sellTit market-font-sell depth-des list fl textL li_num"><span>' + parseInt(key + 1)  + '</span></div>';
					html += '<div class="t_con"><span class="depth-price  s_01 coin_price">' + util.numFormat(priceOffset(Number(value.price), 1), market.cnyDigit) + '</span></div>';
					html += '<div class="t_con"><span class="depth-amount s_01">' + util.numFormat(amountOffset(Number(value.amount), 1), market.coinDigit) + '</span></div>';
...
//				$("#totalCoin").html(util.numFormat(result.data.totalCoin, market.coinDigit));
				...
				$.each(result.fentrusts1, function(key, value) {
					console.log(value);
					if(value.fentrustType == '0'){
						entrutsCur += '<li class="cred"  style="height:0.8rem;border:1px sold white;">';
						entrutsCur += '<em class="fl"><span class="td_01 s_01 coin_price">' + util.numFormat(value.fprize, market.cnyDigit) + '</span></em>';
						entrutsCur += '<em class="cred fl "><span class="td_02 s_01">' + value.fentrustType_s+ '</span></em>';
						...
						entrutsCur += '<em class="cblue2  fl"><span class="entruts-cancel" style="color:#2269d4" data-fid="' + value.fid + '">' + util.getLan("tradeMarket.tips.5") + '</span></em>';
						entrutsCur += '</li>';
					}else{
						entrutsCur += '<li class="cgreen" style="height:0.8rem;border:1px sold white;">';
						entrutsCur += '<em class="fl"><span class="td_01 s_01 coin_price">' + util.numFormat(value.fprize, market.cnyDigit) + '</span></em>';
						entrutsCur += '<em class="cgreen2 fl "><span class="td_02 s_01">' + value.fentrustType_s + '</span></em>';
						entrutsCur += '<em class="fl "><span class="td_02 s_01">' + util.numFormat(value.fcount, market.coinDigit) + '</span></em>';
						entrutsCur += '<em class="fl "><span class="td_03 s_01">' + util.numFormat(value.fprize*value.fcount, market.cnyDigit)+ '</span></em>';
						entrutsCur += '<em class="cblue2 fl"><span class="entruts-cancel" style="color:#2269d4" data-fid="' + value.fid + '">' + util.getLan("tradeMarket.tips.5") + '</span></em>';
						entrutsCur += '</li>';
				...
				if (entrutsCur == "") {
					$("#entrutsCurData").html('<div class="noMsg textCenter mtop"><p style="height: 100%;width: 100%;text-align: center;">' + util.getLan("tradeMarket.tips.6") + '</p></div>');
	...
	numVerify : function(tradeType) {
		if (tradeType == 0) {
			...
			if (tradeTurnover > tradecnymoney) {
				layer.msg(util.getLan("comm.tips.9"),{anim: 'up'});
				return;
			}
			$("#buy-limit").html(util.numFormat(tradeTurnover, market.cnyDigit));
		} else {
	...
	saveCoinTrade : function(tradeType, flag) {
		...
		if (tradePassword == "false") {
			layer.msg(util.getLan("comm.tips.17"),{anim: 'up'});
			return;
		}
		...
		if (isTelephoneBind == "false" && isGoogleBind=="false") {
			layer.msg(util.getLan("comm.tips.1"),{anim: 'up'});
			return;
		}
		...
		if (!reg.test(tradeAmount)) {
			layer.msg(util.getLan("tradeMarket.tips.7"),{anim: 'up'});
			return;
		}

		if (!reg.test(tradePrice)) {
			layer.msg(util.getLan("tradeMarket.tips.10"),{anim: 'up'});
			return;
		}

		var total = util.numFormat(util.accMul(tradeAmount, tradePrice), market.cnyDigit);
		if (parseFloat(total) < 0.00001) {
			layer.msg(util.getLan("tradeMarket.tips.132"),{anim: 'up'});
			return;
		}
		...
		if (tradePwd == "" && isopen == "true") {
			layer.msg(util.getLan("comm.tips.8"),{anim: 'up'});
			$("#isopen").val("true");
			return;
		}
		var url = "";
