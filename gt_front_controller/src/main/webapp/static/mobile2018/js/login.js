...
var login = {
	loginNameOnblur : function() {
		...
		if (!util.checkEmail(uName) && !util.checkMobile(uName)) {
			...
			layer.msg(language["comm.error.tips.1"],{anim: 'up'});
		}
	},
	checkLoginUserName : function() {
		...
		if (uName == "") {
			...
			layer.msg(language["comm.error.tips.5"],{anim: 'up'});
			return false;
	...
	checkLoginPassword : function() {
		...
		if (password == "") {
			...
			layer.msg(language["comm.error.tips.2"],{anim: 'up'});
			return false;
		} else if (password.length < 6) {
			// util.layerTips("login-password", language["comm.error.tips.3"]);
			layer.msg(language["comm.error.tips.3"],{anim: 'up'});
			return false;
		}
		util.hideerrortips("login-errortips");
		return true;
