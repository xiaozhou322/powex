var recharge = {
	submitFinForm : function(that) {
		...
		var money = 0;
		if (sbank == "") {
			util.showMsg(language["comm.error.tips.83"]);
			return;
		}
		...
		if (Number(money) < Number(minMoney) || isNaN(money)) {
			util.showMsg(language["comm.error.tips.84"].format(minMoney));
			return;
		}
	...
	submitTransferAccounts : function() {
		...
		if (bank == "" || bank == "-1" || account == "" || payee == "" || phone == "") {
			util.showMsg(language["comm.error.tips.95"]);
			return;
		}
...
//			}),
	...
	cancelRechargeCNY: function(id) {
        ...
        function(result) {
            ...
            $.post(url, param,
            function(id) {
            	var tt = "rechargecancel_" + it;
            	var show_id = "show" + it;
            	// document.getElementById(tt).innerHTML = "--";
            	// $('#'+tt).text('--').removeClass("rechargecancel");
            	$('#'+tt).remove();
...
//		}
		if (imgcode == "") {
			util.showMsg(language["recharge.error.tips.4"]);
			return;
		}
		money = document.getElementById("diyMoney").value;
		if (money.toString().indexOf(".") != -1) {
