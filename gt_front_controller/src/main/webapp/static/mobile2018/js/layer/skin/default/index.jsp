...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path;if (request.getServerName().equals("www.powex.pro")){basePath="https://www.powex.pro";}
%>
    ...
    <section>
         ...
         <div class="securityMain newsCon helpCon">
             <c:forEach items="${abouts }" var="v" varStatus="vs">
                  ...
                  <span>
                    <span onclick="showtab('coinWarp')">${v.name }</span>
                  </span>
                  </c:if> 
             </c:forEach> 
         </div>
...
<header class="header backHeader">  
    <i class="backIcon"></i>
    <h2 class="tit">${fabout.ftype }</h2>
</header>
<section>
    <div class="passetsMain helpContent">
    <c:forEach items="${abouts }" var="v" varStatus="vs">
         <div class="securityMain newsHelpMain lw-helpCons" >
          <div class="lw-heilpTabs fl" <c:if test="${vs.index ne 0}">style="display:none; width:100%;"</c:if>>
             <ul>  
