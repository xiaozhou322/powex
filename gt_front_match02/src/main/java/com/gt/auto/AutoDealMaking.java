...
public class AutoDealMaking {
	...
	@Autowired
	private TradeDealMaking dealMaking;
	
	public void init() {
		//gai
		List<Ffees> ffees = this.utilsService.list(0, 0, "", false, Ffees.class) ;
		for (Ffees fee : ffees) {
			this.dealMaking.putRates(fee.getFtrademapping().getFid()+"_buy_"+fee.getFlevel(), fee.getFbuyfee()) ;
			this.dealMaking.putRates(fee.getFtrademapping().getFid()+"_sell_"+fee.getFlevel() ,fee.getFfee()) ;
		}
			...
			if (sellLimitFentrusts.length > 0 && buyFentrusts.length > 0) {
				...
				first: for (int i = 0; i < sellLimitFentrusts.length; i++) {
					...
					for (int j = 0; j < buyFentrusts.length; j++) {
						...
						boolean ret = false;
						try {
							dealMaking.updateDealMaking(ftrademapping,buy, sell,
