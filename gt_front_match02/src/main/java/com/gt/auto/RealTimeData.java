		...
		if(null != fentrusts){
			...
			for (Fentrust fentrust : fentrusts) {
				...
				if (index>=pstep){
					...
					stringBuffer.append("["+new BigDecimal(String.valueOf(fentrust.getFprize())).setScale(8, BigDecimal.ROUND_HALF_UP)
			...
			for (Fentrustlog fentrustlog : fentrustlogs) {
				...
				if (index>=pstep){
					...
					stringBuffer.append("["+(fentrustlog.getFcreateTime().getTime())
							+","+fentrustlog.getFid()
							+","+new BigDecimal(String.valueOf(fentrustlog.getFprize())).setScale(8, BigDecimal.ROUND_HALF_UP)
