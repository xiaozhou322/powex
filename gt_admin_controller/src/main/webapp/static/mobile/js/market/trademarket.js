...
//				$("#totalCoin").html(util.numFormat(result.data.totalCoin, market.coinDigit));
				...
				$.each(result.fentrusts1, function(key, value) {
					entrutsCur += '<tr>';
					entrutsCur += '<td class="col-1">' + value.flastUpdatTime_s + '</td>';
					entrutsCur += '<td class="col-3">' + util.numFormat(value.fprize, market.cnyDigit) + '</td>';
					entrutsCur += '<td class="col-3">' + util.numFormat(value.fcount, market.coinDigit) + '</td>';
					entrutsCur += '</tr>';
				});
				if (entrutsCur == "") {
					$("#entrutsCurData").html('<div class="noMsg textCenter mtop"><img src="/static/mobile/images/noMsg.png" width="86" alt="" /><span style="height: 100%;width: 100%;text-align: center;padding-top: 40px;">' + util.getLan("tradeMarket.tips.6") + '</span></div>');
					...
					var entrutsCur1 = '<table>' + entrutsCur +'</table>';
					$("#entrutsCurData").html("").append(entrutsCur1);
					$("#entrutsCurData1").html("").append(entrutsCur1);
				}
