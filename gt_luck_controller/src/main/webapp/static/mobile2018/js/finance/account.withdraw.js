var withdraw = {
	btc : {
		submitWithdrawBtcAddrForm : function() {
			...
			$.post(url, param, function(result) {
				if (result != null) {
					if (result.code == -1) {
						util.showMsg(language["comm.error.tips.34"], {
							okbtn : function() {
	...
	cny : {
		...
		submitWithdrawUsdtForm : function(ele) {
			var withdrawBlank = $("#withdrawBlank").val();
		...
		submitexchangeUsdtForm : function(ele, nvcData) {
...
$(function() {
	...
	$("#withdrawUsdtButton").on("click", function(event) {
		withdraw.usdt.submitWithdrawUsdtForm(this);
	});
	...
	$("#exchangeUsdtButton").on("click", function(event) {
		...
		withdraw.usdt.submitexchangeUsdtForm(this, nvcData);
	});
});
