var market = {
	...
	autoRefresh : function() {
		...
		var tradesCallback = function(result) {
			...
			$("#marketDepthSell").mCustomScrollbar();		// 设置滚动条
			if($("#mCSB_6_container").height()>396){
				$("#mCSB_6_container").css('top',$("#mCSB_6").height()-$("#mCSB_6_container").height());
			}else{
				$("#marketDepthSell").css({
					'position': 'absolute',
					'top':$("#marketDepthSell").height()-$("#mCSB_6_container").height(),
					'width':'100%'
