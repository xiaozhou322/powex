...
public class FmessageDAO extends HibernateDaoSupport {
	...
	public Fmessage findById(java.lang.Integer id) {
		...
		try {
			Fmessage instance = (Fmessage) getSession().get("com.gt.entity.Fmessage", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fmessage> findByExample(Fmessage instance) {
		...
		try {
			List<Fmessage> results = (List<Fmessage>) getSession()
					.createCriteria("com.gt.entity.Fmessage").add(create(instance)).list();
	...
	public List<Fmessage> list(int firstResult, int maxResults, String filter,boolean isFY) {
		List<Fmessage> list = null;
		log.debug("finding Fmessage instance with filter");
		try {
			String queryString = "from Fmessage "+filter;
			Query queryObject = getSession().createQuery(queryString);
			...
			log.error("find by Fmessage name failed", re);
			throw re;
		}
		return list;
	}
