...
public class FarticleDAO extends HibernateDaoSupport  {
    ...
    public List<Farticle> findByExample(Farticle instance) {
        ...
        try {
            List<Farticle> results = (List<Farticle>) getSession()
                    .createCriteria("com.gt.entity.Farticle")
                    .add( create(instance) )
            .list();
	...
	public List<Farticle> findProjectFarticle(int projectId, int farticletype,
			int firstResult, int maxResult) {
		log.debug("findProjectFarticle all Farticle instances");
		try {
			String queryString = "from Farticle where farticletype.fid=? and projectId.fid=? order by fisding desc,id desc";
	         Query queryObject = getSession().createQuery(queryString);
	         queryObject.setCacheable(true) ;
	         queryObject.setParameter(0, farticletype) ;
	         queryObject.setParameter(1, projectId) ;
	         queryObject.setFirstResult(firstResult) ;
	         queryObject.setMaxResults(maxResult) ;
			 return queryObject.list();
		} catch (RuntimeException re) {
	...
	public int findProjectFarticleCount(int projectId, int farticletype) {
		log.debug("findFarticleCount all Farticle instances");
		try {
			String queryString = "select count(*) from Farticle where farticletype.fid=? and projectId.fid=? order by id desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setCacheable(true) ;
			queryObject.setParameter(0, farticletype) ;
			queryObject.setParameter(1, projectId) ;
