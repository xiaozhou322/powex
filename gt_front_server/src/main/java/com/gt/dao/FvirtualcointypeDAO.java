 ...
 * 
 * @see com.gt.entity.Fvirtualcointype
...
public class FvirtualcointypeDAO extends HibernateDaoSupport {
	...
	public Fvirtualcointype findById(java.lang.Integer id) {
		...
		try {
			Fvirtualcointype instance = (Fvirtualcointype) getSession().get(
					"com.gt.entity.Fvirtualcointype", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fvirtualcointype> findByExample(Fvirtualcointype instance) {
		...
		try {
			List<Fvirtualcointype> results = (List<Fvirtualcointype>) getSession()
					.createCriteria("com.gt.entity.Fvirtualcointype")
	...
	//selectType 0是=号，1是<>号
		public List findAll(int coinType1,int coinType2,int selectType) {
			...
			try {
				...
				String queryString = "from Fvirtualcointype where ftype"+eq+"? and ftype"+eq +"?";
				Query queryObject = getSession().createQuery(queryString);
	...
	public List findAllNormal(int selectType) {
		log.debug("finding all Fvirtualcointype instances");
		try {
			String queryString = "from Fvirtualcointype where ftype <> 0 and fstatus=?";
			Query queryObject = getSession().createQuery(queryString);
