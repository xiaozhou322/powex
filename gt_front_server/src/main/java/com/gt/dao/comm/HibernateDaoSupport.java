...
//	  this.hibernateTemplate.setCacheQueries(true) ;//fake hank
	...
	public int findByParamCount(String filter,Class c,Object... param) {
		List list = null;
		try {
	...
	public int min(String filter,String field,Class c,Object... param) {
		try {
			String queryString = "select min("+field+") from "+c.getName()+" "+filter;
			Query queryObject = getSession().createQuery(queryString);
			if(param!=null ){
				for (int i = 0; i < param.length; i++) {
					queryObject.setParameter(i, param[i]) ;
	...
	public int max(String filter,String field,Class c,Object... param) {
		try {
			String queryString = "select max("+field+") from "+c.getName()+" "+filter;
			Query queryObject = getSession().createQuery(queryString);
			if(param!=null ){
				for (int i = 0; i < param.length; i++) {
					queryObject.setParameter(i, param[i]) ;
