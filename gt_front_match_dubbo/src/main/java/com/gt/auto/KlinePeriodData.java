...
public class KlinePeriodData {
	...
	//封装初始化PeriodMap供后台使用
	public void initPeriodMap(Integer id){
		//不存在则进行初始化
		if(!periodMap.containsKey(id)) {
	...
	//封装初始化Period供后台使用
	public void initPeriods(int id){
		//不存在则进行初始化
			...
			for (Fperiod fperiod : fperiods) {
				...
				if (index>=pstep){
					stringBuffer.append("["+(fperiod.getFtime().getTime())
							+","+new BigDecimal(String.valueOf(fperiod.getFkai())).setScale(10, BigDecimal.ROUND_HALF_UP)
							+","+new BigDecimal(String.valueOf(fperiod.getFgao())).setScale(10, BigDecimal.ROUND_HALF_UP)
							+","+new BigDecimal(String.valueOf(fperiod.getFdi())).setScale(10, BigDecimal.ROUND_HALF_UP)
							+","+new BigDecimal(String.valueOf(fperiod.getFshou())).setScale(10, BigDecimal.ROUND_HALF_UP)
