...
//		System.out.println(req.getRequestURL().toString());
		...
		if(	uri.startsWith("/index.html")//首页
			||uri.startsWith("/appWalletApi.html".toLowerCase())  
...
//            ||uri.startsWith("/config/".toLowerCase()) 
            ...
            ||uri.startsWith("/advertisement/buylist".toLowerCase())  //otc用户购买入口不需要登录
			||uri.startsWith("/advertisement/selllist".toLowerCase())  //otc用户出售入口不需要登录
			
			||uri.startsWith("/user/applogin.html".toLowerCase())  //app联合登录
			||uri.startsWith("/introl/introlIndex.html".toLowerCase())  //首页邀请好友
			||uri.startsWith("/setProjectTemplate.html".toLowerCase())  //项目方首页模板设置
           
			|| uri.endsWith("/tv-chart.630b704a2b9d0eaf1593.html".toLowerCase())
			){
